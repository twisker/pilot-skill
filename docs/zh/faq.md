...
**中文** | [English](https://twisker.github.io/pilot-skill/en/faq.html)
...
2. **导出 cookie 后重试**：见 [cookie 指南](https://twisker.github.io/pilot-skill/zh/guide-cookies.html)，成功率提升最明显的一步
...
配置里的 `preferred_domains` 对实证抓取成功率高的域名做了排序加权（见[配置参考](https://twisker.github.io/pilot-skill/zh/guide-config.html)），属预期行为。
...
地图会把**全程有坐标的点按行程顺序连成一条连续路线**（天内、天与天之间都相连，按天用不同颜色区分），单个孤立的点也会与前后相连。注意这是**直线连接、仅示意路线走向，不代表实际道路**——驾车路线需要额外的路由服务（会引入云端依赖），V1 保持本地零依赖，暂不绘制真实路网。
...
cookie 只保存在 `~/.pilot/cookies/`，绝不上传；建议用小号，随时可删。详见 [cookie 指南](https://twisker.github.io/pilot-skill/zh/guide-cookies.html)的红线声明。
