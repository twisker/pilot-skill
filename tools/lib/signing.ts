...
// ---------------------------------------------------------------------------
// Ed25519 签名/验签工具（分发资源完整性校验）
//
// 用途：分发的发布物（自升级包等）由发布流水线签名（detached，base64），
// 客户端内嵌公钥验签，失败拒用。
//
// 沿革：原为商业产品库 products.json 的验签而写。2026-09-24 转免费项目后
// 相关模块已归档 legacy/，本工具被**自升级机制的发布物验签**复用，故保留在
// tools/ 继续维护。
//
// 格式约定：
//   - 公钥：raw 32 字节的 base64（一行常量，便于内嵌代码）
//   - 私钥：PKCS8 PEM 文件（仅存发布机，绝不入库）
