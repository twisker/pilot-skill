[English](README.md) | **中文** | 📖 [文档站](https://twisker.github.io/pilot-skill/)
...
- **透明的匿名统计，可一键关闭**：只记事件计数与目的地粗粒度，绝不采集对话内容、身份信息或设备指纹（`PILOT_TELEMETRY=off` 即全局关闭）——详见[隐私声明](https://twisker.github.io/pilot-skill/zh/privacy.html)

## 界面截图

> 下图为一次真实生成的「新疆伊犁 9 日亲子自驾」路书（6 大 3 小、人均约 ¥9,778）。

**地图伴侣**（本地只读网页，localhost:4870）——全程景点按行程顺序连成一条连续路线，按天配色：

![地图伴侣：全程连续路线](docs/images/web-map.png)

**时间线视图**（逐日逐项，含花费与备注）与**参考游记卡片墙**（精选 5 条，带品味评分）：

![时间线视图](docs/images/web-timeline.png)
![参考游记卡片墙](docs/images/web-cards.png)

**三格式路书导出**——PDF（打印/离线）、Excel（4 sheet，费用可算）、Word（可编辑）：

| PDF | Excel | Word |
|-----|-------|------|
| ![PDF 路书](docs/images/export-pdf.png) | ![Excel 路书](docs/images/export-excel.png) | ![Word 路书](docs/images/export-word.png) |
